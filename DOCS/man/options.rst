...
``--macos-hdr-csp=<type>``
    Activate experimental HDR output color space to utilize macOS X HDR display support.
    "auto" is the default, which will select the color space of the desktop but will not activate HDR mode.
    To activate HDR mode this value must be set specifically matching video's color space.
    It is recommended to use this switch together with ``--target-trc``.
    Both values must correspond to video's color primaries/transfer characteristics 
    in order to get the correct color reproduction.
    ...
    displayP3_PQ_EOTF 
        ...
        ``--target-trc`` must be set to ``pq`` to get the correct video color reproduction
    displayP3_HLG 
